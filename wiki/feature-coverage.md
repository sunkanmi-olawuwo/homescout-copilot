...
| Copilot (agent + endpoint) | Partial | `POST /api/copilot/ask` → `IHomeScoutAgentGateway`; `HomeScoutAgentTools` (mortgage + rental estimators + base rate as `AIFunction`s, verified offline; serves both buyers and renters, prompt `homescout.v3.md`); `FoundryAgentGateway` (Microsoft Agent Framework) **compiles**; DI registers it behind config (503 until Foundry configured); typed `AskCopilotAsync`; offline endpoint tests (fake gateway). **Live agent path not yet verified** — needs `azd provision` + creds. |
