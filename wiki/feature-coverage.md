...
| Copilot agent gateway (tools + shape) | Partial | `IHomeScoutAgentGateway` + `CopilotRequest`/`CopilotAnswer`; `HomeScoutAgentTools` exposes the estimator + base rate as `Microsoft.Extensions.AI` `AIFunction`s (verified offline). Foundry-backed agent (Microsoft Agent Framework) + provisioning pending (Slices 1/3). |
