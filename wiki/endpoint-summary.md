...
| Listing capture (extract) | `POST /api/listings/extract` | Implemented (text + geocode) | Multipart PDF upload (1–4 docs of one property) → `ListingExtractionResult`: a draft `Listing` + per-field provenance/confidence, for the user to confirm. Deterministic text layer (`PdfPigDocumentReader` + `IListingFactParser`) reads labelled fields across Rightmove/Zoopla/OpenRent; the **register cross-check** (`IRegisterCrossCheck`) then verifies the postcode and adds a location via **postcodes.io** (open, keyless — `Provenance=Register`), best-effort and graceful. Never guesses (absent facts → notes). Vision + EPC/council-tax registers are later slices. 400 on non-PDF/empty. Plan: [[Listing Capture — PDF Extraction Pipeline — Design]]. |
